  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update_category':
        ...
        $categories_icon = new upload('icon');
        $categories_icon->set_destination(DIR_FS_CATALOG_IMAGES);

        if ($categories_icon->parse() && $categories_icon->save()) {
          tep_db_query("update " . TABLE_CATEGORIES . " set icon = '" . tep_db_input($categories_icon->filename) . "' where categories_id = '" . (int)$categories_id . "'");
        }
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.icon, c.parent_id, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and cd.categories_name like '%" . tep_db_input($search) . "%' order by c.sort_order, cd.categories_name");
    } else {
...
<?php
    ...
    switch ($action) {
      case 'new_category':
        ...
        $contents[] = array('text' => '<br>' . TEXT_CATEGORIES_ICON . '<br>' . tep_draw_file_field('icon'));
      ...
      case 'edit_category':
        ...
        $contents[] = array('text' => '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $cInfo->icon, $cInfo->categories_name) . '<br>' . DIR_WS_CATALOG_IMAGES . '<br><b>' . $cInfo->icon . '</b>');
        $contents[] = array('text' => '<br>' . TEXT_EDIT_CATEGORIES_ICON . '<br>' . tep_draw_file_field('icon'));
