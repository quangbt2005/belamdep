...
// make sure no duplicate category IDs exist which could lock the server in a loop
  ...
  ////
// Sets the status of a news
  function tep_set_news_status($news_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_NEWS . " set news_status = '1' where news_id = '" . (int)$news_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_NEWS . " set news_status = '0' where news_id = '" . (int)$news_id . "'");
    } else {
      return -1;
    }
  }

  function tep_generate_news_categories_path($id, $from = 'news', $categories_array = '', $index = 0) {

    if (!is_array($categories_array)) $categories_array = array();

    {
      $category_query = tep_db_query("select nc.news_categories_name, nc.parent_id from " . TABLE_NEWS_CATEGORIES . " nc where nc.news_categories_id = '" . (int)$id . "'");
      $category = tep_db_fetch_array($category_query);
      $categories_array[$index][] = array('id' => $id, 'text' => $category['news_categories_name']);
      if ( (tep_not_null($category['parent_id'])) && ($category['parent_id'] != '0') ) $categories_array = tep_generate_news_categories_path($category['parent_id'], 'news', $categories_array, $index);
    }

    return $categories_array;
  }

  function tep_output_generated_news_categories_path($id, $from = 'news') {
    $calculated_category_path_string = '';
  ...
  function tep_get_news_title($news_id) {
    $product_query = tep_db_query("select news_title from " . TABLE_NEWS . " where news_id = '" . (int)$news_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['news_title'];
  }

  function tep_get_news_content($news_id) {
    $product_query = tep_db_query("select news_content from " . TABLE_NEWS . " where news_id = '" . (int)$news_id . "'");
    $product = tep_db_fetch_array($product_query);
...
// Count how many subcategories exist in a category
// TABLES: categories
  function tep_childs_in_news_categories_count($categories_id) {
    $categories_count = 0;

    $categories_query = tep_db_query("select news_categories_id from " . TABLE_NEWS_CATEGORIES . " where parent_id = '" . (int)$categories_id . "'");
    while ($categories = tep_db_fetch_array($categories_query)) {
      $categories_count++;
      $categories_count += tep_childs_in_news_categories_count($categories['news_categories_id']);
    }
...
// TABLES: products, products_to_categories, categories
  ...
  function tep_get_news_categories_path($current_category_id = '') {
    global $cPath_array;
    ...
    if ($current_category_id == '') {
      ...
      if (sizeof($cPath_array) == 0) {
        ...
        $cPath_new = '';
        $last_category_query = tep_db_query("select parent_id from " . TABLE_NEWS_CATEGORIES . " where news_categories_id = '" . (int)$cPath_array[(sizeof($cPath_array)-1)] . "'");
        $last_category = tep_db_fetch_array($last_category_query);

        $current_category_query = tep_db_query("select parent_id from " . TABLE_NEWS_CATEGORIES . " where news_categories_id = '" . (int)$current_category_id . "'");
        $current_category = tep_db_fetch_array($current_category_query);
  ...
  function tep_get_news_categories_name($category_id, $language_id) {
    $category_query = tep_db_query("select news_categories_name from " . TABLE_NEWS_CATEGORIES . " where news_categories_id = '" . (int)$category_id . "'");
    $category = tep_db_fetch_array($category_query);

    return $category['news_categories_name'];
  }

  function tep_get_news_categories_array($category_id, &$ncPath_array)
  {
  	if (!is_array($ncPath_array))	$ncPath_array = array();
  	$ncPath_array[] = $category_id;

  	$query = tep_db_query("select parent_id from " . TABLE_NEWS_CATEGORIES . " where news_categories_id = '" . (int) $category_id. "'");
  	if ($category = tep_db_fetch_array($query))
  	{
  		if ($category['parent_id'] != '0')
  		{
  			tep_get_news_categories_array($category['parent_id'], $ncPath_array);
  		}
  	}

  }

  //this function used when insert new news, it is field news_categories_path
  function tep_get_news_categories_path_toinsert($news_categories_id)
  {
  	$ncPath_array = array();
  	tep_get_news_categories_array($news_categories_id, $ncPath_array);
  	$ncPath_array = array_reverse($ncPath_array);
  	$ncPath  = implode('_', $ncPath_array);
  	$ncPath = '_' . $ncPath . '_';
  	return $ncPath;
  }
?>
